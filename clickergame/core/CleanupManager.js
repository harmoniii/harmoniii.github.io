// core/CleanupManager.js - Fixed version with recursive cleanup protection
export class CleanupManager {
  constructor() {
    ...
    this.isDestroying = false; // FIXED: Add flag to prevent recursive cleanup
  ...
  registerComponent(component, name = 'Unknown') {
    if (this.isDestroyed || this.isDestroying) {
      this.warn('Cannot register component - CleanupManager is destroying/destroyed');
      return false;
    }
    ...
    // FIXED: Check if component is already registered
  ...
  createTimeout(callback, delay, name = 'anonymous') {
    if (this.isDestroyed || this.isDestroying) {
      this.warn('Cannot create timeout - CleanupManager is destroying/destroyed');
      return null;
    }
  ...
  createInterval(callback, delay, name = 'anonymous') {
    if (this.isDestroyed || this.isDestroying) {
      this.warn('Cannot create interval - CleanupManager is destroying/destroyed');
      return null;
    }
  ...
  addEventListener(element, event, handler, options = {}) {
    if (this.isDestroyed || this.isDestroying) {
      this.warn('Cannot add event listener - CleanupManager is destroying/destroyed');
      return () => {};
    }
  ...
  cleanup() {
    ...
    if (this.isDestroying) {
      this.warn('CleanupManager is already destroying, skipping recursive cleanup');
      return;
    }
    
    this.isDestroying = true; // FIXED: Set flag to prevent recursive cleanup
  ...
  cleanupComponents() {
    ...
    // FIXED: Create array copy to avoid modification during iteration
    ...
    componentsArray.forEach(component => {
      try {
        ...
        // FIXED: Check if component has isDestroyed flag and skip if already destroyed
        if (component.isDestroyed === true) {
          this.log(`Component ${name} already destroyed, skipping`);
          this.components.delete(component);
          return;
        }
        
        // FIXED: Set isDestroyed flag before calling destroy to prevent recursion
        ...
        // Remove from components set
        this.components.delete(component);
        
      } catch (error) {
        this.error('Error destroying component:', error);
        // Still remove from set even if destroy failed
        this.components.delete(component);
      }
    });
...
export class CleanupMixin {
  constructor() {
    ...
    this.isDestroyed = false; // FIXED: Add isDestroyed flag
  ...
  destroy() {
    if (this.isDestroyed) {
      return; // FIXED: Prevent multiple destroy calls
    }
