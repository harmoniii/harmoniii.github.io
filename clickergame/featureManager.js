  ...
  applyBuff(def) {
    ...
    if (def.id === 'magnet') {
      this.buffIntervals.magnet = setInterval(() => {
        if (s.combo.lastAngle != null) {
          EventBus.emit('click', s.combo.lastAngle);
        }
      }, 1000);
    }
    if (def.id === 'waterfall') {
      this.buffIntervals.waterfall = setInterval(() => {
        const pool = Object.keys(s.resources).filter(r => r !== 'faith' && r !== 'chaos');
